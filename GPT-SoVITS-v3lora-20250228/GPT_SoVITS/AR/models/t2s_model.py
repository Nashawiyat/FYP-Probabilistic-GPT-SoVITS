...
class Text2SemanticDecoder(nn.Module):
    ...
    def infer(
        ...
        stop = False
        # Collect per-step probability distributions
        prob_history = []
        for _ in tqdm(range(1500)):
            ...
            samples, probs = topk_sampling(
                logits, top_k=top_k, top_p=1.0, temperature=temperature, return_probs=True
            )
            # Store probability distribution for this step
        ...
        # Stack collected probabilities into a sequence tensor (B, T, V)
        probs_seq = torch.stack(prob_history, dim=1)
        return y, probs_seq
