...
cache= {}
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut=i18n("不切"), top_k=20, top_p=0.6, temperature=0.6, ref_free = False,speed=1,if_freeze=False,inp_refs=None,sample_steps=8,if_sr=False,pause_second=0.3, use_probabilistic_tokenization=False):
    global cache
    ...
    for i_text,text in enumerate(texts):
        ...
        if(i_text in cache and if_freeze==True):pred_semantic=cache[i_text]
            ...
            with torch.no_grad():
                if use_probabilistic_tokenization:
                    # Probabilistic AR: collect per-step probabilities
                    pred_semantic_full, prob_seq = t2s_model.model.infer(
                        all_phoneme_ids,
                        all_phoneme_len,
                    ...
                    # Number of generated tokens equals probability steps
                    idx = prob_seq.shape[1]
                    pred_semantic = pred_semantic_full[:, -idx:].unsqueeze(0)
                    prob_semantic = prob_seq
                else:
        ...
        if model_version!="v3":
            ...
            audio = vq_model.decode(pred_semantic, torch.LongTensor(phones2).to(device).unsqueeze(0), refers,speed=speed, prob_semantic=prob_semantic)[0][0]#.cpu().detach().numpy()
        else:
    ...
    with gr.Group():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=7):
                ...
                use_probabilistic_tokenization = gr.Checkbox(label=i18n("Using probabilistic semantic sampling (comparative experiment)"), value=False, interactive=True, show_label=True, scale=1)
            # with gr.Column():
        ...
        inference_button.click(
            get_tts_wav,
            [inp_ref, prompt_text, prompt_language, text, text_language, how_to_cut, top_k, top_p, temperature, ref_text_free, speed, if_freeze, inp_refs, sample_steps, if_sr_Checkbox, pause_second_slider, use_probabilistic_tokenization],
            [output],
        )
