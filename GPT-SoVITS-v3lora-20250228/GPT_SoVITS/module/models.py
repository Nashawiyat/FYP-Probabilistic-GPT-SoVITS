...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Linear Adapter: refine 768-dim mixture \tilde{z} -> 768
        # Small, trainable adapter included in checkpoints
        self.prob_adapter = nn.Linear(ssl_dim, ssl_dim, bias=False)
        self._debug_prob_step = 0

    def forward(self, ssl, y, y_lengths, text, text_lengths, prob_semantic: torch.Tensor = None):
        y_mask = torch.unsqueeze(commons.sequence_mask(y_lengths, y.size(2)), 1).to(
            y.dtype
        )
        if(self.version=="v1"):
        ...
        with autocast(enabled=False):
            ...
            if prob_semantic is not None:
                # Step A: handle EOS mismatch (1025 -> 1024) and renormalize
                probs = prob_semantic
                if probs.size(-1) == 1025:
                    probs = probs[..., :1024]
                    denom = probs.sum(dim=-1, keepdim=True)
                    probs = probs / torch.clamp(denom, min=1e-8)
                # Step B: decode to 768-dim mixture \tilde{z}
                quantized = self.quantizer.vq.decode_soft(probs)
                # Step C: pass through small linear adapter (apply on last dim)
                if self.training:
                    quantized = self.prob_adapter(quantized.transpose(1, 2)).transpose(1, 2)
                else:
                    # For inference with pre-trained models that don't have adapter weights yet
                    pass
                if (
                    self.training
                    ...
                    if self._debug_prob_step % 100 == 0:
                        print(
                            f"DEBUG: Prob Token Sample (First 5 dims): {prob_semantic[0, 0, :5]}"
                        )
                    self._debug_prob_step += 1
                # No commitment loss when using external soft tokens
                commit_loss = torch.tensor([0.0], device=quantized.device, dtype=quantized.dtype)
            else:
    ...
    @torch.no_grad()
    def decode(self, codes, text, refer, noise_scale=0.5, speed=1, prob_semantic: torch.Tensor = None):
        def get_ge(refer):
            ge = None
        ...
        # Determine target lengths from codes or probabilistic semantics
        if prob_semantic is not None:
            y_lengths = torch.LongTensor([prob_semantic.size(1) * 2]).to((prob_semantic.device))
        else:
            y_lengths = torch.LongTensor([codes.size(2) * 2]).to(codes.device)
        text_lengths = torch.LongTensor([text.size(-1)]).to(text.device)

        # Obtain semantic embeddings from discrete codes or probabilities
        if prob_semantic is not None:
            # Step A: handle EOS mismatch (1025 -> 1024) and renormalize
            probs = prob_semantic
            if probs.size(-1) == 1025:
