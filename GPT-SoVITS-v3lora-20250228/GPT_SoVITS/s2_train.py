...
def run(rank, n_gpus, hps):
    ...
    # Derive model_type from probabilistic setting and prepare stats path
    model_type = "Probabilistic" if float(getattr(hps.train, "probabilistic_rate", 0.0)) > 0.0 else "Baseline"
    stats_path = os.path.join(hps.data.exp_dir, "training_stats.txt")
    os.makedirs(hps.data.exp_dir, exist_ok=True)

    print("start training from epoch %s" % epoch_str)
    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0:
            # Synchronize GPU before starting timer to ensure all kernels complete
            if torch.cuda.is_available():
            ...
            # Synchronize GPU and stop timer after epoch work finishes
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            duration = time.perf_counter() - t_start

            # Console print
            print(f"Epoch {epoch} duration: {duration:.6f} sec ({model_type})")

            # Persistent append-only logging
            try:
                with open(stats_path, "a", encoding="utf-8") as f:
                    f.write(f"epoch={epoch}\tmodel_type={model_type}\tduration_sec={duration:.6f}\n")
            except Exception as e:
                try:
                    logger.info(f"Failed to write training_stats.txt: {e}")
                except Exception:
                    pass
        else:
            train_and_evaluate(
...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        # Optionally enable probabilistic path using soft tokens derived from current batch
        use_prob = torch.rand(1).item() < getattr(hps.train, 'probabilistic_rate', 0.1)
        prob_semantic = None
        if use_prob:
            # Project SSL features to match semantic frame rate before quantization
            if torch.cuda.is_available() and hasattr(net_g, 'module'):
                ssl_proj = net_g.module.ssl_proj(ssl)
                _, codes, _, _ = net_g.module.quantizer(ssl_proj, layers=[0])
                bins = net_g.module.quantizer.bins
            else:
                ssl_proj = net_g.ssl_proj(ssl)
                _, codes, _, _ = net_g.quantizer(ssl_proj, layers=[0])
                bins = net_g.quantizer.bins
            # Use the single quantizer layer indices
            codes = codes[0]
            # Build soft distributions with light label smoothing to avoid one-hot collapse
            eps = getattr(hps.train, 'prob_label_smoothing', 0.05)
            prob_semantic = F.one_hot(codes.long(), num_classes=bins).float()
            prob_semantic = prob_semantic * (1.0 - eps) + (eps / bins)
        ...
        with autocast(enabled=hps.train.fp16_run):
            (
                ...
                stats_ssl,
            ) = net_g(ssl, spec, spec_lengths, text, text_lengths, prob_semantic=prob_semantic)
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            with autocast(enabled=False):
                ...
                # When probabilistic path is active, kl_ssl (commit_loss) is set to 0 in the model
                # to avoid collapsing back to one-hot; otherwise include it as usual
